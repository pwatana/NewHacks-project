...
def learn_cpts(model, df):
    ...
    categorical_columns = ['Electricity', 'Water', 'Food', 'House', 'Temporary Shelter', 'Injuries']
    ...
    # Reorganize the DataFrame to match the structure required by the Bayesian network
    df_reorganized = pd.DataFrame()
    for household in range(1, 11):
        for var in categorical_columns + numerical_columns:
            df_reorganized[f'Household {household}_{var}'] = df[var]

    # Learn CPTs using Maximum Likelihood Estimation
    model.fit(df_reorganized, estimator=MaximumLikelihoodEstimator)
    return model
